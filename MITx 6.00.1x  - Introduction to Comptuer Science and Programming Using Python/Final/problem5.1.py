...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        """
        for courseObject in self.myCourses:
            if course == courseObject.courseName:
                courseObject.setGrade(grade)
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        for courseObject in self.myCourses:
            if course == courseObject.courseName:
                courseObject.setPset(pset,score)
    ...
    def getPset(self, pset, course="6.00x"):
        ...
        for courseObject in self.myCourses:
            if course == courseObject.courseName:
                return courseObject.getPset(pset)
        return -1

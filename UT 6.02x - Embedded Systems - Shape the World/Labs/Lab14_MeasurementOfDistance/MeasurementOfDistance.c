...
unsigned long Mail;				// mailbox data
unsigned long Status = 0;			// mailbox status flag 1 is full, 0 is empty
...
unsigned long Convert(unsigned long sample){
	...
	while(Status == 0) { /*do nothing*/ } // while status is empty
	
	Status = 0;
	if((2000.0/4095.0*sample - (int)(2000.0/4095.0*sample)) > 0.5) {
		return 2000.0/4095.0*sample+1; // round up
	}else {
		return 2000.0/4095.0*sample;
	}
...
void SysTick_Init(unsigned long period){
	...
	NVIC_ST_RELOAD_R = period; // Setup reload to be the period
...
// Reverse string used in UART_ConvertDistance
void ReverseString(unsigned char stringToReverse[], unsigned int length) {
	
	int i;
	for(i = 0; i < length/2; i++){
		
		char temp = stringToReverse[i];
		stringToReverse[i] = stringToReverse[length - 1 - i];
		stringToReverse[length - 1 - i] = temp;
